...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetAppSlots` (IN `appDate` DATE)   WITH morningCount AS 
(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetSalesReport` (IN `startDate` VARCHAR(50), IN `endDate` VARCHAR(50))   SELECT
	SUM(total_price) AS Total
FROM tbl_billing 
WHERE order_status = 'Complete' AND place_on BETWEEN startDate AND endDate$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetServicesReport` (IN `startDate` VARCHAR(50), IN `endDate` VARCHAR(50))   SELECT SUM(total_cost) As Total FROM tbl_appointment WHERE status = 'complete' AND appointment_started BETWEEN startDate AND endDate$$
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `productstockchart`  AS SELECT `tbl_product`.`product_category` AS `product_category`, sum(`tbl_product`.`product_stock`) AS `ProductStockCount` FROM `tbl_product` GROUP BY `tbl_product`.`product_category` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `users`  AS SELECT `tbl_user`.`name` AS `name`, `tbl_user`.`email` AS `email`, `tbl_user`.`number` AS `number`, `tbl_user`.`address` AS `address`, `tbl_user`.`status` AS `status` FROM `tbl_user` ;
