...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Nov 24, 2023 at 02:02 AM
...
--
-- Database: `u572443248_motojen_db`
--
...
-- Procedures
--
CREATE DEFINER=`u572443248_motojen`@`127.0.0.1` PROCEDURE `spGetAppSlots` (IN `appDate` DATE)   WITH morningCount AS 
(
...
CREATE DEFINER=`u572443248_motojen`@`127.0.0.1` PROCEDURE `spGetSalesReport` (IN `startDate` VARCHAR(50), IN `endDate` VARCHAR(50))   SELECT
	SUM(total_price) AS Total
FROM tbl_billing 
WHERE order_status = 'Complete' AND place_on BETWEEN startDate AND endDate$$

CREATE DEFINER=`u572443248_motojen`@`127.0.0.1` PROCEDURE `spGetServicesReport` (IN `startDate` VARCHAR(50), IN `endDate` VARCHAR(50))   SELECT SUM(total_cost) As Total FROM tbl_appointment WHERE status = 'complete' AND appointment_started BETWEEN startDate AND endDate$$
...
(39, 0, 'tune up,carborador cleaning,change seat cover with seat cover set,PALIT TAMBUCHO,', '2023-11-25', 'afternoon', 'Complete', 2300);
...
(1, 26, 'Ayemimaw', 958612345, 'Gcash', 'sample address', 'YUASA BATTERY YTX4L-BS MOTORCYCLE BATTERY (MF4L-B) SPEEDMOTO  (900 x 1) - ', 900, '2023-11-18 20:38:39', 900, 'Uservalid-Pic9108.jpg', 0, 'Full', '12345678913654521', 'Complete'),
...
(29, 22, 'joshua', 9123456789, 'pickup', 'novaliches, quezon city', 'YUASA BATTERY YTX4L-BS MOTORCYCLE BATTERY (MF4L-B) SPEEDMOTO  (900 x 1) - OD Battery Gel Type YTX4L-', 1650, '2023-11-22 21:54:13', 1650, '', 0, 'Full', '', 'Complete'),
(30, 22, 'joshua', 9123456789, 'Gcash', 'novaliches, quezon city', 'Motolite MOTORCYCLE Battery (VRLA Maintenance Free) MF4L-B  (700 x 1) - ', 700, '2023-11-23 05:03:23', 700, 'Uservalid-Pic8398.jpeg', 0, 'Full', '858147066', 'Complete'),
(31, 22, 'joshua', 9123456789, 'Gcash', 'novaliches, quezon city', 'Nibbi Racing Throttle Cable (350 x 1) - ', 350, '2023-11-23 07:29:00', 350, 'Uservalid-Pic9245.jpg', 0, 'Full', '123456789', 'Complete');
...
(47, 22, 1, 'Motolite MOTORCYCLE Battery (VRLA Maintenance Free) MF9-B  ', 1250, 1, 'Product-Motojen-7905.png');
...
(8, 'Motolite MOTORCYCLE Battery (VRLA Maintenance Free) MF4L-B ', 700, 'Product-Motojen-7362.png', 'battery', 46),
...
(29, 'Makoto bearing 6201', 30, 'Product-Motojen-9111.png', 'Bearing', 50),
(31, 'Makoto bearing 6203', 40, 'Product-Motojen-1754.png', 'Bearing', 50),
...
(110, 'Nibbi Racing Throttle Cable', 350, 'Nibbi Racing Throttle Cable.png', 'Throttle Cable', 99),
(111, 'Moto Depot Throttle Cable RXT135 1 Piece', 120, 'Moto Depot Throttle Cable RXT135 1 Piece.png', 'Throttle Cable', 55);
...
(34, 3, 'Domino Quick Throttle Racing with Cable', 210, 'Domino Quick Throttle Racing with Cable.png', 'Throttle Cable', 250);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u572443248_motojen`@`127.0.0.1` SQL SECURITY DEFINER VIEW `productstockchart`  AS SELECT `tbl_product`.`product_category` AS `product_category`, sum(`tbl_product`.`product_stock`) AS `ProductStockCount` FROM `tbl_product` GROUP BY `tbl_product`.`product_category` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u572443248_motojen`@`127.0.0.1` SQL SECURITY DEFINER VIEW `users`  AS SELECT `tbl_user`.`name` AS `name`, `tbl_user`.`email` AS `email`, `tbl_user`.`number` AS `number`, `tbl_user`.`address` AS `address`, `tbl_user`.`status` AS `status` FROM `tbl_user` ;
...
--
ALTER TABLE `tbl_audit_trail`
  MODIFY `audit_id` int(100) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=88;
...
--
ALTER TABLE `tbl_supplier_products`
  MODIFY `tbl_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=36;
